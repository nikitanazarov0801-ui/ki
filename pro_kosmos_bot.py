...
# -------------------- POST SANITIZER (REMOVE " and * + ADD EMOJI) --------------------
def sanitize_post(text: str, title_emoji: str = "🚀") -> str:
    # Remove forbidden chars
    text = (text or "").replace('"', "").replace("*", "")

    lines = [ln.rstrip() for ln in text.splitlines()]

    # Emoji to title (first line)
    if lines and lines[0].strip() and not lines[0].lstrip().startswith(title_emoji):
        lines[0] = f"{title_emoji} {lines[0].strip()}"

    def find_idx(prefixes: list[str]):
    ...
    idx_facts = find_idx(["факты:"])
    idx_why = find_idx(["почему важно:"])
    idx_next = find_idx(["что дальше:"])
    idx_src = find_idx(["источник:"])

    def prefix_header(i: int | None, em: str):
        if i is None:
            return
        ln = lines[i].strip()
        if not ln.startswith(em):
            lines[i] = f"{em} {ln}"

    prefix_header(idx_facts, "🛰️")
    prefix_header(idx_why, "❗")
    prefix_header(idx_next, "🔭")
    prefix_header(idx_src, "🔗")

    # Normalize bullets (•/—/-) into '- '
    out = []
    for ln in lines:
        s = ln.lstrip()
        if s.startswith(("-", "•", "—")):
...
def init_db():
    # APScheduler can run jobs in a different thread -> allow cross-thread use,
    # and serialize operations with DB_LOCK.
...
def fetch_feed(feed_url: str):
    r = requests.get(feed_url, headers={"User-Agent": RSS_UA}, timeout=(5, RSS_TIMEOUT))
    r.raise_for_status()
...
def extract_image_url(entry, link: str) -> str | None:
    ...
    if link:
        try:
            r = requests.get(link, headers={"User-Agent": RSS_UA}, timeout=(5, PAGE_TIMEOUT))
            r.raise_for_status()
...
def expand_image_variants(url: str) -> list[str]:
    """
    NASA images-assets often uses name~large.jpg -> try smaller ones first.
    """
    if not url:
        return []
    ...
    if "images-assets.nasa.gov" in url and "~large" in url:
        variants = [url.replace("~large", "~medium"), url.replace("~large", "~small"), url]
...
def generate_post(c):
    ...
    # sanitize: remove " and * + add emojis
...
def run_job():
    ...
    try:
        ...
        # keep only candidates with image URL (mp4 ignored)
        candidates_img = []
        for c in candidates:
        ...
        for c in ordered:
            ...
            # 1) prepare photo first (cheap compared to YandexGPT)
            prepared = prepare_photo_for_candidate(c)
            if not prepared:
                # do NOT mark seen: CDN failure may be temporary
                continue

            photo_bytes, fname = prepared

            # 2) generate text only if photo is ready
            post_text = generate_post(c)

            # 3) send; if sendPhoto fails, mark seen to avoid infinite loops
